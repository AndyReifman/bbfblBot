...
public class LaunchWindow {
    ...
    start.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        if (btnFirstDraft.isSelected()) {
          ...
          JOptionPane.showMessageDialog(null, "Please ensure you choose the number of rounds");
        }
        if (temp.size() > 1) {
  ...
  public ArrayList<Integer> pullInfo(JTextField txtOrder, int numRounds) {
    ...
    try {
      ...
      if(txtOrder.getText().equals(""))
        JOptionPane.showMessageDialog(null, "Please enter a draft order");
      else
        JOptionPane.showMessageDialog(null, "Please enter only numbers and spaces");
      txtOrder.setText("");
    }
  ...
  private void connect(ArrayList<Integer> order, int rounds) {
    ...
    try {
      ...
      // bot.sendMessage("nickserv", "IDENTIFY <password>"); //Once username is registered, this
